...
- IP: VPS_HOST_FROM_CRED_FILE, user: root, pass: OLD_VPS_PASS
...
- БД seeded: Business 'Barber Ekaterina' (id 04d1be59-422a-4f92-88bf-242031f14d82)
  ...
  down → up → продолжить).
- Бот: @My_Barber_hair_bot (telegram), token OLD_BOT_TOKEN
...
Этап 5 — verify + deploy:
  ...
  5.4 — Deploy на Timeweb: ssh root@VPS_HOST_FROM_CRED_FILE → git pull → docker compose
        down → up -d --build
