...
- IP: VPS_HOST_FROM_CRED_FILE, user: root, pass: <VPS_PASSWORD в 1Password/secrets manager>
  NB: пароль ранее был в этом файле в публичном git history (commit 7194163) —
  СКОМПРОМЕТИРОВАН. Сменить через ssh root@VPS_HOST_FROM_CRED_FILE → passwd (Шаг 2).
- Команды: ssh root@VPS_HOST_FROM_CRED_FILE
...
**Что случилось:** VPS пароль `OLD_VPS_PASS` + bot token
`OLD_BOT_TOKEN` + Postgres пароль
...
1. ✅ VPS пароль сменён через `ssh root@VPS_HOST_FROM_CRED_FILE` → `passwd`.
   Старый пароль мёртв.
