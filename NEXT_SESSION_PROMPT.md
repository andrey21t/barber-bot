...
> VPS deploy через ssh root@VPS_HOST_FROM_CRED_FILE (password rotated 27 авг, нет в .env).
...
```bash
ssh root@VPS_HOST_FROM_CRED_FILE "cd /opt/barber-bot && git pull && docker compose up -d --build && docker compose restart bot && docker compose logs bot --tail 20"
```
