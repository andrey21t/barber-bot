...
Coverage (AUTONOMOUS_COVERAGE_PROMPT.md T6, 2026-09-13 update):
- /start from admin → welcome (без имени, универсально для любого мастера) +
  admin_reply_keyboard (4 кнопки: 📋 Меню / 📅 Сегодня / 🗓 Неделя / ❌ Отмена)
- /start from client → client welcome message + client_reply_keyboard (2 кнопки)

Pattern: direct handler invocation with mock Message + User. No FSM. No DB
(2026-09-13: master.name DB lookup убран из /start — вариант A, универсальный
welcome, не зависит от данных БД).

Session 2026-09-13 (универсальный welcome): 4 fallback-теста удалены —
master_not_found / db_error / escapes_html / empty_name. DB lookup
(_resolve_master_name) больше не вызывается, тесты на fallback стали мёртвыми.
"""
...
async def test_cmd_start_admin_shows_welcome_and_reply_keyboard() -> None:
    """/start from admin (ADMIN_ID) → welcome (без имени) + always-on reply
    keyboard (4 кнопки).

    Session 2026-09-13 (универсальный welcome, вариант A): убран master.name
    DB lookup — greeting «Привет! 👋» для любого мастера, не зависит от данных
    БД. 4-я кнопка «❌ Отмена» — universal escape hatch из mid-FSM
    (donor-ресёрч: winnerxxx13, UznetDev — dedicated cancel button не standard
    ...
    reply keyboard (📋 Меню / 📅 Сегодня / 🗓 Неделя). 2026-09-13 добавлена
    4-я кнопка ❌ Отмена.
    """
    ...
    # 1 вызов answer (welcome + reply keyboard)
    msg.answer.assert_awaited_once()
    state.clear.assert_awaited_once()  # W1 fix: /start clears FSM state

    text = _answer_text(msg)
    # 2026-09-13: welcome без имени мастера (универсальный)
    assert "Привет! 👋" in text, "greeting must be universal (no master name)"
    assert "Екатерина" not in text, "greeting must NOT hardcode name"
